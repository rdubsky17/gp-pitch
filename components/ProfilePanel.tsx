...
export default function ProfilePanel() {
  ...
  const [scoresTab, setScoresTab] = useState<'lastScores' | 'scoresBySong'>('lastScores');
  ...
  const fetchScores = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Print scores in browser console to debug
        console.log(JSON.stringify(data.scores, null, 2));

        setScores(data.scores.slice(0, 10));
      }
    } catch (error) {
        ...
        <div>
          ...
          {(
            <div style={{ padding: '40px 20px', textAlign: 'center', color: '#666' }}>
              <div style={{ fontSize: 16, marginBottom: 8 }}>Not implemented yet</div>
            </div>
          )}
        </div>
      )}
